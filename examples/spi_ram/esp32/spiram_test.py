...
def test():
  s=spiram()
  for i in range(10):
    s.led.on();s.hwspi.write(bytearray([0,0x00,0x00+i,0x39-i]));s.led.off()

  for i in range(10):
    s.led.on();s.hwspi.write(bytearray([1,0x00,0x00+i,0]));print(s.hwspi.read(1));s.led.off()
  s.led.on();s.hwspi.write(bytearray([1,0x00,0x00,0]));print(s.hwspi.read(10));s.led.off()

  s.led.on();s.hwspi.write(bytearray([0,0x00,0x00,0x30,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39]));s.led.off()
  s.led.on();s.hwspi.write(bytearray([1,0x00,0x00,0]));print(s.hwspi.read(10));s.led.off()

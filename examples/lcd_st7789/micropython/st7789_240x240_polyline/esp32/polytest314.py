...
class test:
  def __init__(self):
    ...
    self.spi = SPI(2, baudrate=20*1000000, polarity=0, phase=0, bits=8, firstbit=SPI.MSB,\
      mosi=Pin(25), sck=Pin(27), miso=Pin(22))
  ...
  def main(self):
    ...
    self.vf.text("ABCČĆDĐEFG",0,50,spacing=22,xscale=1024,yscale=1024,color=st7789poly.rgb565(255,255,0))
    self.vf.text("abcčćdđefg",0,80,spacing=22,xscale=1024,yscale=1024,color=st7789poly.rgb565(255,255,0))
    self.vf.text("0123456789",0,110,spacing=22,xscale=1024,yscale=1024,color=st7789poly.rgb565(0,255,255))
    self.vf.text("@!?#$%&\\~^|",0,140,spacing=22,xscale=1024,yscale=1024,color=st7789poly.rgb565(0,255,255))
    self.vf.text("8888888888888888888888888888888",0, 170)
    self.vf.text("@!?#$%&\\~^|",0,180,color=st7789poly.rgb565(255,255,255))
    self.vf.text("Accelerated polyline core!",0,190,color=st7789poly.rgb565(255,255,255))
    #for i in range(n):
